...
#[test]
fn registers_root_qualified_typed_constants_from_rbis() {
    let source = std::fs::read_to_string("tests/fixtures/root_qualified_typed_constant.rb")
        .expect("fixture exists");
    let mut files = load_workspace_paths(&builtin_rbi_paths().expect("vendored RBIs load"))
        .expect("vendored RBIs load");
    files.push(WorkspaceFile::new(
        "root_qualified_typed_constant.rb",
        source,
    ));
    let result = check_workspace(&files, CheckerConfig::default());
    assert!(!result.has_errors(), "{:#?}", result.diagnostics);
...
#[test]
fn transfers_source_line_through_owned_hir() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_source_line.rb").unwrap();
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_rescue_modifier_through_owned_hir() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_rescue_modifier.rb").unwrap();
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_back_reference_reads_through_owned_hir() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_back_reference.rb").unwrap();
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_multi_write_call_targets_through_owned_hir() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_multi_write_targets.rb").unwrap();
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn preserves_unresolved_super_gradual_result() {
...
#[test]
fn transfers_owned_literals_and_reads_without_changing_results() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_owned_values.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn advances_after_logical_positional_splat_values_in_owned_cfg() {
    let source =
        std::fs::read_to_string("tests/fixtures/cfg_positional_splat_logical.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn preserves_optional_parameter_defaults_in_owned_cfg() {
    let source =
        std::fs::read_to_string("tests/fixtures/cfg_optional_default_flow.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
fn transfers_owned_collection_trees_without_prism_child_evaluation() {
    ...
    assert!(
        ...
        "missing concrete collection type: {:?}",
        cfg.types
    );
}

#[test]
fn transfers_global_array_coercion_through_shared_intrinsic_semantics() {
    let source =
        std::fs::read_to_string("tests/fixtures/cfg_array_coercion_union.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn dispatches_runtime_constant_indexers_through_owned_signatures() {
    let source =
        std::fs::read_to_string("tests/fixtures/cfg_runtime_constant_indexer.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_ensure_on_normal_and_exceptional_paths_without_changing_results() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_ensure_transfer.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_symbol_passed_blocks_with_their_method_return_type() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_symbol_block.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_loop_bodies_through_the_owned_cfg_graph() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_loop_body.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_for_bodies_through_owned_collection_iteration() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_for_body.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn transfers_explicit_method_returns_through_the_owned_cfg_graph() {
    let source = std::fs::read_to_string("tests/fixtures/cfg_explicit_return.rb").expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
#[test]
fn merges_possible_rescue_local_state() {
    let source =
        std::fs::read_to_string("tests/fixtures/rescue_local_flow.rb").expect("fixture exists");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
        CheckerConfig {
...
fn preserves_builtin_models_through_owned_cfg_transfer() {
    for path in [
        ...
        "tests/fixtures/cfg_rbi_constant_alias.rb",
        "tests/fixtures/class_object_subclass_narrowing.rb",
...
#[test]
fn preserves_attached_class_intrinsics_through_owned_cfg_transfer() {
    let path = "tests/fixtures/attached_class_usage.rb";
    let source = std::fs::read_to_string(path).expect("fixture source");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
...
#[test]
fn preserves_enumerable_assignability_through_owned_cfg_transfer() {
    let path = "tests/fixtures/set_enumerable_assignability.rb";
    let source = std::fs::read_to_string(path).expect("fixture source");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
...
#[test]
fn preserves_dynamic_class_eval_through_owned_cfg_transfer() {
    let path = "tests/fixtures/dynamic_class_include_and_eval.rb";
    let source = std::fs::read_to_string(path).expect("fixture source");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
...
#[test]
fn transfers_common_object_methods_through_owned_cfg() {
    let path = "tests/fixtures/cfg_common_object_methods.rb";
    let source = std::fs::read_to_string(path).expect("fixture");
    let baseline = check(&source, CheckerConfig::default());
    let cfg = check(
        &source,
...
#[test]
fn models_ruby_argv_as_strings() {
    let result = check_fixture("tests/fixtures/argv_type.rb");
    assert!(!result.has_errors(), "{:?}", result.diagnostics);
    assert!(result.diagnostics.iter().any(|diagnostic| diagnostic
        .message
        .contains("Revealed type: `T::Array[String]`")));
