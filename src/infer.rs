...
use method_types::{
    apply_parameter_shape, merge_method_signatures, optional_proc_type, proc_arity_narrowing,
    proc_parts, proc_receiver, ParameterShape,
};
...
impl<'src> Analyzer<'src> {
    ...
    fn assignment_value_node<'node>(&self, node: &Node<'node>) -> Option<Node<'node>> {
    ...
    fn assignment_receiver_node<'node>(&self, node: &Node<'node>) -> Option<Option<Node<'node>>> {
    ...
    fn assignment_index_arguments<'node>(
        &self,
        node: &Node<'node>,
